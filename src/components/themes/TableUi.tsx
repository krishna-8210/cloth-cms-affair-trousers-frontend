...
const TableRawCmp=({item,columns,item_list}:any)=>{
...
function TableUi({columns,ListCmp,list_api}:any) {
    ...
    const listLoader = async () => {
        console.log(page)
        try {
            const resp_list = await responseHandler(list_api, { id:'', data: '', query: `page=${page}` })
    ...
    return (
        ...
        >
            ...
            <TableBody items={list.list}>
                {(item: any) => (
                 <ListCmp item={item} columns={columns} item_list={list.list}/>
